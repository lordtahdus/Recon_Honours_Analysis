...
visnights_raw <- readr::read_csv(
    here::here("tourism/data/visnights_monthly.csv")
  ) |>
...
(eigen(cov(resid))$values[1:20]/1e6) %>%
  plot(type = "b",
       xlab = "Component",
       ylab = "Eigenvalue ('000,000)"
  )
...
eigenvals_bot <- eigen(cor(resid[1:120, colnames(S)]))$values
eigenvals_bot[1:50] |> plot()
nScree(x = eigenvals_bot)$Components


library(rmfd4dfm)         # Bai–Ng IC implementations
baingcriterion(resid[, colnames(S)], rmax = 20)   # one of the ICs
```
...
start_date <- visnights_full$Month[1]
end_date <- start_date + 120 - 1
M <- 133
seq_dates <- list(
  start = start_date + 0:(M-1),
  end = end_date + 0:(M-1)
)
saveRDS(seq_dates, file = "tourism/job/seq_dates.rds")
